# OverLanguage 2.0: GlyphForge, SQGN, and Layer4Meter
...
1. Visual plane: screen checkpoints, active windows, UI state, visible artifacts.
2. File plane: file deltas, hashes, build artifacts, cache growth.
3. Process plane: process spawns, terminal commands, app execution events.
4. Power/performance plane: CPU time, disk writes, memory pressure, network bytes, thermal behavior.
5. Time/snapshot plane: before/after state, APFS or Time Machine-style anchors, artifact existence proof.
...
## SQGN: SubQuantum Glyph Notation

SubQuantum Glyph Notation is a symbolic language for representing invisible layers beneath a normal file: event traces, deltas, hidden compute, hashes, location anchors, receipts, proof states, and transferability.

Boundary: **subquantum here does not mean literal physics.** It means below the visible artifact layer: the smallest evidence particles that make a file, app, receipt, or asset provable.

Core taxonomy:

```text
Layer 0: ∅       null / absence
Layer 1: q       subquantum evidence particles
Layer 2: □ ◇ ⧉   objects and artifacts
Layer 3: H M     identity and Merkle commitment
Layer 4: L T Ω   location, time, canonical source
Layer 5: R σ ZK  receipts, signatures, proofs
Layer 6: λ λ⁻¹ Λ friction and transferability
Layer 7: χ LCI   hidden substrate compute
Layer 8: ◎ ✕ ?   verification state
Layer 9: $ V D ρ financeability and revenue readiness
```

Validity rules:

```text
H requires bytes or a hash source.
L requires an anchor.
R requires evidence.
λ requires measurement or declared scoring method.
◎ requires verifier result.
$ requires buyer, use case, price, or demand signal.
ZK requires actual proof mechanics.
```
...
- GitHub REST API supports creating and updating UTF-8 repository contents through the contents API.
- GitHub issues can be used later to break this spec into implementation tickets.
- Hashing should be treated as irreversible identity commitment, not file reconstruction.
- Base64 should be treated as reversible transport encoding, not proof.
